...
public class Force implements Runnable {
	...
	public Vector3f secondMethod(Vector3f tmp, float dt, int tier, int ifPlayer) { // from
		...
		if (ifPlayer == 1) {
			...
			// forcetmp.div(99999);
			// force.add(forcetmp);
			// System.out.print(dataObject.getPlayer().getExtraForce().x);
			// System.out.print(dataObject.getPlayer().getExtraForce().y);
			// System.out.print(dataObject.getPlayer().getExtraForce().z);
	...
	public synchronized void calculateDeltas() { // approximation
		...
		while (isRunning) {
			...
			k1v[dataObject.getAsteroids().size()]
					.add(secondMethod(position[dataObject.getAsteroids().size()], dt, 0, 1));
			firstUpdate();
			...
			k2v[dataObject.getAsteroids().size()]
					.add(secondMethod(position[dataObject.getAsteroids().size()], dt, 1, 1));
			secondUpdate();
			...
			k3v[dataObject.getAsteroids().size()]
					.add(secondMethod(position[dataObject.getAsteroids().size()], dt, 1, 0));
			thirdUpdate();
			...
			k4v[dataObject.getAsteroids().size()]
					.add(secondMethod(position[dataObject.getAsteroids().size()], dt, 2, 1));
			fourthUpdate();
			...
			for (int ii = 0; ii < dataObject.getAsteroids().size() + 1; ii++) {
				if (ii < dataObject.getAsteroids().size()) {
					...
					/*
					 * Vector3f tmp=new Vector3f(k1r[ii]); tmp.add(k2r[ii]);
					 * tmp.add(k2r[ii]); tmp.add(k3r[ii]); tmp.add(k3r[ii]);
					 * tmp.add(k4r[ii]); tmp.div(6);
					 * dataObject.getPlayer().increasePosition(tmp);
					 * 
					 * tmp=new Vector3f(k1v[ii]); tmp.add(k2v[ii]);
					 * tmp.add(k2v[ii]); tmp.add(k3v[ii]); tmp.add(k3v[ii]);
					 * tmp.add(k4v[ii]); tmp.div(6);
			...
			try {
				Thread.sleep((long) (Timer.getLastLoopTime() - end));
