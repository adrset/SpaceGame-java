...
public class Menu {
	...
	public Menu() {
		...
		ui.getLayer(id2).addButton(Game.loader.loadTexture("GUI1"), Game.loader.loadTexture("GUI2"),
				new Vector2f(0f, 0.05f), new Vector2f(0.2f, 0.1f), "Antyaliasing x" + Game.getMultiSampling(), arial);
		...
		ui.getLayer(id2).getButton(2).addListener(() -> {
			...
			ui.getLayer(id2).getButton(2).changeText("Antyaliasing x" + samples[(seek + 1) % 5]);
		});
